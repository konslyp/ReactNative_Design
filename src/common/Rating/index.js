...
export default class Rating extends PureComponent {
  ...
  renderFullStars() {
    return [...Array(this.state.fullStars)].map(
      (num, i) => <Icon name="star" key={`${i}-fullstar`} color={this.props.color} size={this.state.size} />
    );
  }

  renderEmptyStars() {
    return [...Array(this.state.emptyStars)].map(
     (num, i) => <Icon key={`${i}-emptystar`} name="star-outline" color={this.props.color} size={this.state.size} />
    );
  }
